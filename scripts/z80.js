...
Z80Module = function () {
	...
	Z80Exports.decodeZ80 = function (mmu, addr) {
		...
		if (prefix1 == 0xCB) {
			...
			throw "invalid " + toHex8(opcode) + "x:" + ox + "y:" + oy + "z:" + oz + "p:" + op + "q:" + oq;
		}
		if (prefix1 == 0xED) {
			...
			if (oz == 3) {
				fetchnWord();
				if (oq === 0) return ["LD (" + toHex16(onn) + ")," + tableRP[op], idx];
			...
			throw "not implemented " + toHex8(opcode) + " x:" + ox + "y:" + oy + "z:" + oz + "p:" + op + "q:" + oq;
		}
	...
	Z80State.prototype.getIX = function (offset) {
		var addr = this.IXH << 8 | this.IXL;
		if (offset) return (addr + offset) & 0xFFFF;
		return addr;
	};
	...
	Z80.prototype.push16 = function (val) {
		//console.log("push16: " + toHex16(this._s.getSP()) + " val: " + toHex16(val));
	...
	Z80.prototype.pop16 = function () {
		...
		this._s.setSP(SP + 1);
		//console.log("pop16 : " + toHex16(this._s.getSP()) + " val: " + toHex16(val));
		return val;
	};
	...
	Z80.prototype._opcodes = {
		...
		0x02:function () { // LD (BC),A
			this._op_t = 7;
			this._op_m = 1;
			this._mmu.w8(this._s.getBC(), this._s.A);
		},
		...
		0x0A:function () { // LD A,(BC)
			this._op_t = 7;
			this._op_m = 1;
			this._s.A = this._mmu.r8(this._s.getBC());
		},
		...
		0x12:function () { // LD (DE),A
			this._op_t = 7;
			this._op_m = 1;
			this._mmu.w8(this._s.getDE(), this._s.A);
		},
		...
		0x27:function () { // DAA
			...
			if (this._s.getF(F_N)) { // sub
				res = sub8(this._s.A, diff, 0);
				this._s.A = res.val;
			}
			else { // add
				res = add8(this._s.A, diff, 0);
				this._s.A = res.val;
			}
...
/*
			...
			var res = add8(this._s.A, addthis, 0);
			this._s.A = res.val;
			this._s.setF(
				F_S, res.F_S,
				F_Z, res.F_Z,
...
*/
		...
		0x2A:function () { // LD HL,(nn)
			...
			this._op_m = 3;
			var addr = this._mmu.r16(this._s.getPC(1));
			this._s.setHL(this._mmu.r16(addr));
		},
		...
		0x80:function () { // ADD A,B
			this._op_t = 4;
			this._op_m = 1;
			var res = add8(this._s.A, this._s.B, 0);
			this._s.A = res.val;
			this._s.updateF(res);
		},
		0x81:function () { // ADD A,C
			this._op_t = 4;
			this._op_m = 1;
			var res = add8(this._s.A, this._s.C, 0);
			this._s.A = res.val;
			this._s.updateF(res);
		},
		0x82:function () { // ADD A,D
			this._op_t = 4;
			this._op_m = 1;
			var res = add8(this._s.A, this._s.D, 0);
			this._s.A = res.val;
			this._s.updateF(res);
		},
		0x83:function () { // ADD A,E
			this._op_t = 4;
			this._op_m = 1;
			var res = add8(this._s.A, this._s.E, 0);
			this._s.A = res.val;
			this._s.updateF(res);
		},
		0x84:function () { // ADD A,H
			this._op_t = 4;
			this._op_m = 1;
			var res = add8(this._s.A, this._s.H, 0);
			this._s.A = res.val;
			this._s.updateF(res);
		},
		0x85:function () { // ADD A,L
			this._op_t = 4;
			this._op_m = 1;
			var res = add8(this._s.A, this._s.L, 0);
			this._s.A = res.val;
			this._s.updateF(res);
		},
		0x86:function () { // ADD A,(HL)
			this._op_t = 7;
			this._op_m = 1;
			var res = add8(this._s.A, this._mmu.r8(this._s.getHL()), 0);
			this._s.A = res.val;
			this._s.updateF(res);
		},
		0x87:function () { // ADD A,A
			this._op_t = 4;
			this._op_m = 1;
			var res = add8(this._s.A, this._s.A, 0);
			this._s.A = res.val;
			this._s.updateF(res);
		},
		...
		0xCE:function () { // ADC	A,n
			...
			this._op_m = 2;
			var val = this._mmu.r8(this._s.getPC(1));
			var res = add8(this._s.A, val, this._s.getF(F_C));
			this._s.A = res.val;
			this._s.updateF(res);
		},
		...
		0xDD09:function () { // ADD IX,BC
			this._op_t = 15;
			this._op_m = 2;
			var res = add16(this._s.getIX(), this._s.getBC());
			this._s.setIX(res.val);
			this._s.setF(F_H, res.F_H, F_N, false, F_C, res.F_C);
		},
		0xDD19:function () { // ADD IX,DE
			this._op_t = 15;
			this._op_m = 2;
			var res = add16(this._s.getIX(), this._s.getDE());
			this._s.setIX(res.val);
			this._s.setF(F_H, res.F_H, F_N, false, F_C, res.F_C);
		},
		0xDD21:function () { // LD IX,nn
			this._op_t = 14;
			this._op_m = 4;
			this._s.setIX(this._mmu.r16(this._s.getPC(2)));
		},
		0xDD22:function () { // LD (nn),IX
			this._op_t = 20;
			this._op_m = 4;
			var addr = this._mmu.r16(this._s.getPC(2));
			this._mmu.w16(addr, this._s.getIX());
		},
		...
		0xDD26:function () { // LD IXH,n*
			this._op_t = 7;
			this._op_m = 3;
			this._s.IXH = this._mmu.r8(this._s.getPC(2));
		},
		0xDD29:function () { // ADD IX,IX
			this._op_t = 15;
			this._op_m = 2;
			var res = add16(this._s.getIX(), this._s.getIX());
			this._s.setIX(res.val);
			this._s.setF(F_H, res.F_H, F_N, false, F_C, res.F_C);
		},
		...
		0xDD2E:function () { // LD IXL,n*
			this._op_t = 7;
			this._op_m = 3;
			this._s.IXL = this._mmu.r8(this._s.getPC(2));
		},
		...
		0xDD35:function () { // DEC (IX+d)
			...
			var displ = this._mmu.r8s(this._s.getPC(2));
			var addr = this._s.getIX(displ);
			var res = sub8(this._mmu.r8(addr), 1);
			this._mmu.w8(addr, res.val);
			this._s.setF(
				F_S, res.F_S,
				F_Z, res.F_Z,
		...
		0xDD39:function () { // ADD IX,SP
			this._op_t = 15;
			this._op_m = 2;
			var res = add16(this._s.getIX(), this._s.getSP());
			this._s.setIX(res.val);
			this._s.setF(F_H, res.F_H, F_N, false, F_C, res.F_C);
		},
		...
		0xDD46:function () { // LD B,(IX+d)
			this._op_t = 19;
			this._op_m = 3;
			var disp = this._mmu.r8(this._s.getPC(2));
		...
		0xDD5E:function () { // LD E,(IX+d)
			...
			this._op_m = 3;
			var disp = this._mmu.r8(this._s.getPC(2));
			this._s.E = this._mmu.r8(this._s.getIX(disp));
		},
		...
		0xDD66:function () { // LD H,(IX+d)
			...
			this._op_m = 3;
			var disp = this._mmu.r8(this._s.getPC(2));
			this._s.H = this._mmu.r8(this._s.getIX(disp));
		},
		...
		0xDD6E:function () { // LD L,(IX+d)
			...
			this._op_m = 3;
			var disp = this._mmu.r8(this._s.getPC(2));
			this._s.L = this._mmu.r8(this._s.getIX(disp));
		},
		...
		0xDD7E:function () { // LD A,(IX+d)
			...
			this._op_m = 3;
			var disp = this._mmu.r8(this._s.getPC(2));
			this._s.A = this._mmu.r8(this._s.getIX(disp));
		},
		0xDD84:function () { // ADD A,IXH*
			//TODO: fix timing info
			this._op_t = 4;
			this._op_m = 2;
			var res = add8(this._s.A, this._s.IXH, 0);
			this._s.A = res.val;
			this._s.updateF(res);
		},
		...
		0xDD86:function () { // ADD A,(IX+d)
			...
			this._op_m = 3;
			var displ = this._mmu.r8s(this._s.getPC(2));
			var res = add8(this._s.A, this._mmu.r8(this._s.getIX(displ)), 0);
			this._s.A = res.val;
			this._s.updateF(res);
		},
		0xDD8C:function () { // ADC A,IXH*
			...
			this._op_t = 4;
			this._op_m = 2;
			var res = add8(this._s.A, this._s.IXH, this._s.getF(F_C));
			this._s.A = res.val;
			this._s.updateF(res);
		},
		...
		0xDD94:function () { // SUB IXH*
			...
			this._op_t = 4;
			this._op_m = 2;
			var res = sub8(this._s.A, this._s.IXH);
			this._s.A = res.val;
			this._s.updateF(res);
		},
		...
		0xDD9C:function () { // SBC A,IXH*
			...
			this._op_t = 4;
			this._op_m = 2;
			var res = sub8(this._s.A, this._s.IXH, this._s.getF(F_C));
			this._s.A = res.val;
			this._s.updateF(res);
		},
		...
		0xDDA4:function () { // AND IXH*
			...
			this._op_t = 4;
			this._op_m = 2;
			this._s.A = this._s.A & this._s.IXH;
			this._s.setF(
				F_S, (this._s.A & 0x80) !== 0,
				F_Z, this._s.A === 0,
				F_H, true,
		...
		0xDDAC:function () { // XOR IXH*
			...
			this._op_t = 4;
			this._op_m = 2;
			this._s.A = (this._s.A ^ this._s.IXH) & 0xFF;
			this._s.setF(
				F_S, (this._s.A & 0x80) !== 0,
				F_Z, this._s.A === 0,
				F_H, false,
		...
		0xDDE3:function () { // EX (SP),IX
			this._op_t = 23;
			this._op_m = 2;
			var addr = this._s.getSP();
			var memval = this._mmu.r16(addr);
			this._mmu.w16(addr, this._s.getIX());
			this._s.setIX(memval);
		},
		...
		0xDE:function () { // SBC	A,n
			this._op_t = 7;
			this._op_m = 2;
			var res = sub8(this._s.A, this._mmu.r8(this._s.getPC(1)), this._s.getF(F_C));
			this._s.A = res.val;
			this._s.updateF(res);
		},
		...
		0xED4A:function () { // ADC	HL,BC
			this._op_t = 15;
			this._op_m = 2;
			var res = add16(this._s.getHL(), this._s.getBC(), this._s.getF(F_C));
			this._s.setHL(res.val);
			this._s.updateF(res);
		},
		...
		0xED5A:function () { // ADC	HL,DE
			this._op_t = 15;
			this._op_m = 2;
			var res = add16(this._s.getHL(), this._s.getDE(), this._s.getF(F_C));
			this._s.setHL(res.val);
			this._s.updateF(res);
		},
		...
		0xED67:function () { // RRD
			...
			this._mmu.w8(HL, (this._s.A << 4) | (memval >>> 4));
		...
		0xED6A:function () { // ADC	HL,HL
			this._op_t = 15;
			this._op_m = 2;
			var res = add16(this._s.getHL(), this._s.getHL(), this._s.getF(F_C));
			this._s.setHL(res.val);
			this._s.updateF(res);
		},
		0xED6B:function () { //
		...
		0xED6F:function () { // RLD
			...
			var HL = this._s.getHL()
			var memval = this._mmu.r8(HL);
			this._mmu.w8(HL, (memval << 4) | (this._s.A & 0x0F));
		...
		0xED7A:function () { // ADC	HL,SP
			this._op_t = 15;
			this._op_m = 2;
			var res = add16(this._s.getHL(), this._s.getSP(), this._s.getF(F_C));
			this._s.setHL(res.val);
			this._s.updateF(res);
		},
		0xED7B:function () { // LD	SP,(nn)
			...
			this._op_m = 4;
			var addr = this._mmu.r16(this._s.getPC(2));
			this._s.setSP(this._mmu.r16(addr));
		},
		0xED7C:function () { //
			this._op_t = 0;
			this._op_m = 0;
		...
		0xEDA1:function () { // CPI
			this._op_t = 16;
			this._op_m = 2;
			var res = sub8(this._s.A, this._mmu.r8(this._s.getHL()));
			this._s.setHL(this._s.getHL() + 1);
			this._s.setBC(this._s.getBC() - 1);
			this._s.setF(
				F_S, res.F_S,
		...
		0xFD21:function () { // LD IY,nn
			this._op_t = 14;
			this._op_m = 4;
			this._s.setIY(this._mmu.r16(this._s.getPC(2)));
		},
		0xFD22:function () { // LD (nn),IY
			this._op_t = 20;
			this._op_m = 4;
			var addr = this._mmu.r16(this._s.getPC(2));
			this._mmu.w16(addr, this._s.getIY());
		},
		...
		0xFD26:function () { // LD IYH,n*
			this._op_t = 7;
			this._op_m = 3;
			this._s.IYH = this._mmu.r8(this._s.getPC(2));
		},
		...
		0xFD2E:function () { // LD IYL,n*
			this._op_t = 7;
			this._op_m = 3;
			this._s.IYL = this._mmu.r8(this._s.getPC(2));
		},
		...
		0xFD5E:function () { // LD E,(IY+d)
			...
			this._op_m = 3;
			var disp = this._mmu.r8(this._s.getPC(2));
			this._s.E = this._mmu.r8(this._s.getIY(disp));
		},
		...
		0xFD66:function () { // LD H,(IY+d)
			...
			this._op_m = 3;
			var disp = this._mmu.r8(this._s.getPC(2));
			this._s.H = this._mmu.r8(this._s.getIY(disp));
		},
		...
		0xFD6E:function () { // LD L,(IY+d)
			...
			this._op_m = 3;
			var disp = this._mmu.r8(this._s.getPC(2));
			this._s.L = this._mmu.r8(this._s.getIY(disp));
		},
		...
		0xFD7E:function () { // LD A,(IY+d)
			...
			this._op_m = 3;
			var disp = this._mmu.r8(this._s.getPC(2));
			this._s.A = this._mmu.r8(this._s.getIY(disp));
		},
		0xFD84:function () { // ADD A,IYH*
			//TODO: fix timing info
			this._op_t = 4;
			this._op_m = 2;
			var res = add8(this._s.A, this._s.IYH, 0);
			this._s.A = res.val;
			this._s.updateF(res);
		},
		...
		0xFD86:function () { // ADD A,(IY+d)
			...
			this._op_m = 3;
			var displ = this._mmu.r8s(this._s.getPC(2));
			var res = add8(this._s.A, this._mmu.r8(this._s.getIY(displ)), 0);
			this._s.A = res.val;
			this._s.updateF(res);
		},
		0xFD8C:function () { // ADC A,IYH*
			...
			this._op_t = 4;
			this._op_m = 2;
			var res = add8(this._s.A, this._s.IYH, this._s.getF(F_C));
			this._s.A = res.val;
			this._s.updateF(res);
		},
		...
		0xFD94:function () { // SUB IYH*
			...
			this._op_t = 4;
			this._op_m = 2;
			var res = sub8(this._s.A, this._s.IYH);
			this._s.A = res.val;
			this._s.updateF(res);
		},
		...
		0xFD9C:function () { // SBC A,IYH*
			...
			this._op_t = 4;
			this._op_m = 2;
			var res = sub8(this._s.A, this._s.IYH, this._s.getF(F_C));
			this._s.A = res.val;
			this._s.updateF(res);
		},
		...
		0xFDA4:function () { // AND IYH*
			...
			this._op_t = 4;
			this._op_m = 2;
			this._s.A = this._s.A & this._s.IYH;
			this._s.setF(
				F_S, (this._s.A & 0x80) !== 0,
				F_Z, this._s.A === 0,
				F_H, true,
		...
		0xFDAC:function () { // XOR IYH*
			...
			this._op_t = 4;
			this._op_m = 2;
			this._s.A = (this._s.A ^ this._s.IYH) & 0xFF;
			this._s.setF(
				F_S, (this._s.A & 0x80) !== 0,
				F_Z, this._s.A === 0,
				F_H, false,
		...
		0xFDBE:function () { // CP (IY+d)
			...
			this._op_m = 3;
			var displ = this._mmu.r8s(this._s.getPC(2));
			var rhs = this._mmu.r8(this._s.getIY(displ));
			var res = sub8(this._s.A, rhs);
			this._s.updateF(res);
		},
...
// //////////////////////////////////////////
	...
	FakeMMU.prototype.w8 = function (addr, val) {
		this._mem[addr] = val;
	};
	FakeMMU.prototype.r8s = function (addr) {
		var val = this._mem[addr];
		if (val & 0x80) return -1 * (0x80 - (val & 0x7f));
		return val;
	};
	FakeMMU.prototype.r16 = function (addr) {
		return (this.r8(addr + 1) << 8) | this.r8(addr);
	};
	FakeMMU.prototype.w16 = function (addr, val) {
		this.w8(addr + 1, val >>> 8);
		this.w8(addr, val & 0xFF);
	};

	Z80Exports.testCpu2 = function () {
		var fakeMmu = new FakeMMU();
		var valIdx = 0x1000;
		var valIdy = 0x1100;
		var opIdx = 0x100;
		var z80 = new Z80(fakeMmu, null, null);
	...
	Z80Exports.testCpu1 = function (skipCnt, doDasm) {
		var fs = require("fs");
		var fakemmu = new FakeMMU();

		var testData = fs.readFileSync("zexdoc.com");
		var i;
	...
	if (process.argv.length > 2) {
		if (process.argv[2] == "cpu") {
			if (process.argv.length < 5) {
				throw("ERROR: cpu do_disasm skip_cnt");
			}
			Z80Module.testCpu1(parseInt(process.argv[4],10), parseInt(process.argv[3],10));
		}
		if (process.argv[2] == "cpu2") {
			Z80Module.testCpu2();
		}
	}
}
